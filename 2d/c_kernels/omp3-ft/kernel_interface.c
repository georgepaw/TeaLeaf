...
void run_cg_calc_w(Chunk* chunk, Settings* settings, double* pw)
{
  ...
  if(do_FT_check)
  {
    cg_calc_w_check(chunk->x, chunk->y,
              settings->halo_depth, pw, chunk->p, chunk->w, chunk->kx, chunk->ky);
...
void run_cheby_init(Chunk* chunk, Settings* settings)
{
  ...
  cheby_init(
    ...
    chunk->p, chunk->r, chunk->w, chunk->kx, chunk->ky);
  STOP_PROFILING(settings->kernel_profile, __func__);
}
  ...
  Chunk* chunk, Settings* settings, double alpha, double beta)
{
  ...
  cheby_iterate(
    ...
    chunk->u, chunk->u0, chunk->p, chunk->r, chunk->w, chunk->kx, chunk->ky);
  STOP_PROFILING(settings->kernel_profile, __func__);
}
  ...
  Chunk* chunk, Settings* settings, double alpha, double beta)
{
  ...
  ppcg_inner_iteration(
    ...
    chunk->r, chunk->sd, chunk->kx, chunk->ky);
...
{
  START_PROFILING(settings->kernel_profile);
  matrix_check(chunk->x, chunk->y, settings->halo_depth, chunk->kx, chunk->ky);
  STOP_PROFILING(settings->kernel_profile, __func__);
}
