        ...
        double* non_zeros)
{
    ...
    // const int x = x_inner + 2*halo_depth-1;
    // const int y = y_inner + 2*halo_depth-1;
    // const int col = gid % x_inner;
    // const int row = gid / x_inner;
    // const int off0 = halo_depth*(x + 1);
    // const int index = off0 + col + row*x;
    ...
    const int off0 = 0;
    const int index = off0 + col + row*x;
    int coef_index = row_index[index];

    if (row <    halo_depth || col <    halo_depth ||
        row >= y-halo_depth || col >= x-halo_depth) return;

    non_zeros[coef_index]   = -ky[index];
    col_index[coef_index++] = index-x;

    non_zeros[coef_index]   = -kx[index];
    col_index[coef_index++] = index-1;

    non_zeros[coef_index]   = (1.0 +
                               kx[index+1] + kx[index] +
                               ky[index+x] + ky[index]);
    col_index[coef_index++] = index;

    non_zeros[coef_index]   = -kx[index+1];
    col_index[coef_index++] = index+1;

    non_zeros[coef_index]   = -ky[index+x];
    col_index[coef_index++] = index+x;
}
		...
		double* rro)
{
	...
	if(gid < x_inner*y_inner)
	{
        ...
        {
            smvp += non_zeros[idx] * u[col_index[idx]];
        }

        w[index] = smvp;
        r[index] = u[index] - smvp;
        p[index] = r[index];
        // printf("%d %u %u %lf %lf %lf dataaaaa\n", index, row_begin, row_end, w[index], r[index], p[index]);
        // printf("%d %lf dataaaaa\n", index, smvp);
        ...
        double* pw)
{
    ...
    if(gid < x_inner*y_inner)
    {
        ...
        for (uint32_t idx = row_begin; idx < row_end; idx++)
        {
            smvp += non_zeros[idx] * p[col_index[idx]];
        }
