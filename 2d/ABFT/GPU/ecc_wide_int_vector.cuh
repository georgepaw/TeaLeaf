...
#elif defined(ABFT_METHOD_INT_VECTOR_SECDED128)
  uint64_t * all_bits = (uint64_t*)rows_in;
  uint64_t parity = __parityll(all_bits[0]) ^ __parityll(all_bits[1]);
...
__device__ static inline void add_ecc_int(uint32_t * rows_out, const uint32_t * rows_in)
{
  ...
  rows_out[1] |= __parityll(all_bits) << 31;
#elif defined(ABFT_METHOD_INT_VECTOR_SECDED128)
  uint64_t * all_bits = (uint64_t*)rows_in;
  //just use Hamming code
  const int secded_bits[] =
  {
    __parityll((S1_ECC_128BITS_lower & all_bits[0]) ^ (S1_ECC_128BITS_upper & all_bits[1])),
    __parityll((S2_ECC_128BITS_lower & all_bits[0]) ^ (S2_ECC_128BITS_upper & all_bits[1])),
    __parityll((S3_ECC_128BITS_lower & all_bits[0]) ^ (S3_ECC_128BITS_upper & all_bits[1])),
    __parityll((S4_ECC_128BITS_lower & all_bits[0]) ^ (S4_ECC_128BITS_upper & all_bits[1])),
    __parityll((S5_ECC_128BITS_lower & all_bits[0]) ^ (S5_ECC_128BITS_upper & all_bits[1])),
    __parityll((S6_ECC_128BITS_lower & all_bits[0]) ^ (S6_ECC_128BITS_upper & all_bits[1])),
    __parityll((S7_ECC_128BITS_lower & all_bits[0]) ^ (S7_ECC_128BITS_upper & all_bits[1])),
    __parityll((S8_ECC_128BITS_lower & all_bits[0]) ^ (S8_ECC_128BITS_upper & all_bits[1]))
  };
  uint32_t secded_parts[] =
  {
    (  secded_bits[0] << 28 | secded_bits[1] << 29
  ...
  rows_out[2] |= (__parityll(all_bits[0]) ^ __parityll(all_bits[1])) << 31;
#endif
}
