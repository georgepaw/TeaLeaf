...
#define CHECK_ECC(col, val, a_cols, a_non_zeros, idx, fail_function)
#define CHECK_CRC32C(cols, vals, a_col, a_non_zeros, row_begin, jj, kk, fail_function)\
if(1){ \
  /*CRC32C TeaLeaf Specific*/\
  if(!check_correct_crc32c_bits(cols, vals, a_col, a_non_zeros, row_begin, 5))\
  {\
    fail_function;\
  }\
} else
...
__device__ const uint32_t crc32c_table[8][256] =
{
  {
...
__device__ static inline uint32_t crc32c_software_simple(uint32_t crc, const uint8_t * data, size_t num_bytes)
{
  while (num_bytes--)
  {
...
__device__ static inline uint32_t generate_crc32c_bits(uint32_t * a_cols, double * a_non_zeros, uint32_t num_elements)
{
  uint32_t crc = 0xFFFFFFFF;
  //remove masks
  crc = crc32c_software_split(crc, a_cols, sizeof(uint32_t) * num_elements);
  crc = crc32c_software_split(crc, (uint32_t*)a_non_zeros, sizeof(double) * num_elements);
...
__device__ static inline uint8_t check_correct_crc32c_bits(uint32_t * cols, double * vals, uint32_t * a_cols, double * a_non_zeros, uint32_t idx, uint32_t num_elements)
{
  ...
  uint32_t current_crc = generate_crc32c_bits(cols, vals, num_elements);
...
__device__ static inline void assign_crc32c_bits(uint32_t * a_cols, double * a_non_zeros, uint32_t idx, uint32_t num_elements)
{
  if(num_elements < 4)
  {
